...
import { useState, SetStateAction, Suspense } from 'react';
import { socialIcon, headerMenu, marqueeInfo } from '../static-data';
import { usePathname, useRouter, useSearchParams } from 'next/navigation';
import { IoCartOutline } from 'react-icons/io5';
import { BiBell } from 'react-icons/bi';
                  ...
                  >
                    ...
                    >
                      {headerMenu.map((val, index) => (
                        <li
                          key={index}
                          className={`cursor-pointer px-4 ${val.name === headerNav ? 'text-[#F25E26]' : 'text-[#A09F9F]'} hover:text-[#F25E26] ${!isOpen ? 'py-2 lg:py-1' : ''}`}
                          onClick={() => {
                            setActiveMenu(activeMenu === index ? null : index);
                            setHeaderNav(val.name);
                          }}
