                ...
                <ul className="relative pt-6 bottom-4 z-10">
                    {
                        categories.map((val, index) => (
                            <Fragment key={index}>
                                ...
                                {active === index && (
                                    <div className={`${inter.className} z-20 bg-white ${active === 1 ? "flex gap-3  -top-0 -right-32" : "w-[60%] -top-4 -right-12"} shadow-md rounded text-sm absolute`}>

                                        {val.categories?.map((subcategory) => (
                                            <div key={subcategory.name} className={` ${active === 1 ? "" : "hover:bg-[#FCDFD4]"} my-4 cursor-pointer p-2 z-20`}>
                                                <Link href={"#"} className={` `} onClick={() => handleSubClick(subcategory.name)}>{subcategory.name}</Link>
